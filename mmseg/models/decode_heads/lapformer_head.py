...
#Model 1: [1/32, 1/4, 1/8, 1/16] => cat(x[i], x[i - 1]) and remove PPM module
@HEADS.register_module()
class LAPFormerHead_new_1_rev_PPM(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 pool_scales=(1, 2, 3, 6),
                 ppm_chans=128,
    ...
    def forward(self, inputs):
        ...
        # [1/4, 1/8, 1/16, 1/32] => [1/32, 1/4, 1/8, 1/16]
        _inputs.insert(0, _inputs.pop(-1))
        # slow concatenate
        _out = torch.empty(
            _inputs[0].shape
        )
