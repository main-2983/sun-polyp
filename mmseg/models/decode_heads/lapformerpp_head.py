...
@HEADS.register_module()
class ELAPFormerPPHead_v2(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 act_cfg=dict(type='GELU'),
                 **kwargs):
        super(ELAPFormerPPHead_v2, self).__init__(
            input_transform='multiple_select',
            act_cfg=act_cfg,
            **kwargs
        )
...
@HEADS.register_module()
class FLAPFormerHead(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 **kwargs):
        ...
        self.highres_path = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(
    ...
    def forward(self, inputs):
        ...
        for idx in range(len(_inputs) - 1, 0, -1):
            ...
            x = torch.cat([x1, x2], dim=1)
            _out = linear_prj(x)
            # CSP-ized highest res featmaps
            if idx == 1:
                _out1 = self.highres_path[0](_out)
                _out2 = self.highres_path[1](_out)
                outs.extend([_out1, _out2])
            else:
                outs.append(_out)
...
@HEADS.register_module()
class LAPFormerPPHead_v4(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 **kwargs):
        super(LAPFormerPPHead_v4, self).__init__(
            input_transform='multiple_select',
            **kwargs
        )
        self.interpolate_mode = interpolate_mode
        ...
        self.multi_scale = MultiScaleLocalAttn(
            channels=self.channels * (num_inputs - 1)
        )
