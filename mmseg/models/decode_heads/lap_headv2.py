...
# Add attention gate to last add in v2_34
@HEADS.register_module()
    ...
    def forward(self, inputs):
        ...
        for idx in range(len(inputs) -1, -1, -1):
            ...
            if idx == len(inputs) - 1:
                ...
                x1 = out
                x2 = inputs[idx - 1]
            if idx == 0: # attention gate for 1/32 + 1/4
                x = self.attn_gate(x2, x1)
            else:
                x = x1 + x2
            out = linear_prj(x)
            outs.append(out)
...
# attention gate weighting to last add in v2_34
@HEADS.register_module()
...
@HEADS.register_module()
class ScaledLAPFormerv2(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 dw_kernel_size=3,
                 scale=0,
        ...
        self.convs = nn.ModuleList()
        for i in range(num_inputs):
            if i < 3 - scale: # scale = 0 -> except last stage, scale = 1 -> except last 2
                self.convs.append(
                    ConvModule(
                        in_channels=self.in_channels[i],
                        out_channels=self.channels,
