...
# Add PPM Attention v2 to last scale
@HEADS.register_module()
class LAPHead_v2_8(BaseDecodeHead):
    def __init__(self,
        ...
        # PPM Attention
        self.ppm = PyramidPoolingAttentionv2(
            channels=self.in_channels[-1],
            act_cfg=attn_act,
            pool_scales=pool_scales
        )
...
class LAPHead_v2_9(BaseDecodeHead):
    def __init__(self,
        ...
        self.fusion_conv = ConvModule(
            ...
            kernel_size=1,
            norm_cfg=self.norm_cfg)
        self.scale = Scale(self.channels)
...
# Change all concat to add with scale
@HEADS.register_module()
...
# PPA + Scaling residual, however, scaling residual is derived from
# other part, this acts like an attention
@HEADS.register_module()
class LAPHead_v2_11(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        # perform identity mapping, scale is derived from out
        out = self.scale(out) * outs[-1] + out
...
# SE -> TripletAttention
@HEADS.register_module()
class LAPHead_v2_12(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 attn_act=None,
                 pool_scales=(1, 2, 3, 6),
        ...
        self.se_module = TripletAttention(
           kernel_size=triplet_kernel,
            act_cfg=triplet_act
        )
        self.fusion_conv = ConvModule(
            in_channels=self.channels * num_inputs,
...
class LAPHead_v2_13(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        inputs[-1] = resize(input=inputs[-1],
                            ...
                            align_corners=self.align_corners)
        # forward triplet
        inputs[-1] = self.triplet(inputs[-1])
...
class LAPHead_v2_14(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        inputs[-1] = resize(input=inputs[-1],
                            ...
                            align_corners=self.align_corners)
        # forward attn
        inputs[-1] = self.spatial_attn(inputs[-1])
...
# Drop concatenation, SE Module, Skip-connection
# Keep PPA only
@HEADS.register_module()
