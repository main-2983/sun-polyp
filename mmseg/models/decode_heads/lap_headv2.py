...
# ExFuse + Add + Full-Scale
# Cat all -> cat first and last
@HEADS.register_module()
class LAPHead_v2_v12(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 **kwargs):
        super().__init__(
    ...
    def forward(self, inputs):
        ...
        out = torch.cat([outs[-1], outs[-2]], dim=1)
        out = self.se_module(out)
        out = self.fusion_conv(out)
        # perform identity mapping
        out = outs[-2] + out
...
# Cat all -> add all
@HEADS.register_module()
class LAPHead_v2_13(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 **kwargs):
        super().__init__(
...
# ExFuse + Add + Full-Scale
# 1/32 FRM -> NormStripConv v1
@HEADS.register_module()
class LAPHead_v2_14(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 **kwargs):
        super().__init__(
...
# ExFuse + Add + Full-Scale
# 1/32 FRM -> NormStripConv v2
@HEADS.register_module()
class LAPHead_v2_15(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 **kwargs):
        super().__init__(
...
# ExFuse + Add + Full-scale
# Add spatial attention after PFF
@HEADS.register_module()
class LAPHead_v2_16(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 spatial_attn_kernel=7,
                 spatial_attn_act=dict(
                     type='Sigmoid'
                 ),
                 spatial_attn_norm=None,
                 **kwargs):
        super().__init__(
            input_transform='multiple_select',
            **kwargs
        ...
        for i in range(num_inputs):
            ...
            self.spatial_attns.append(
                AvgSpatialAttn(
                    channels=self.channels,
                    kernel_size=spatial_attn_kernel,
                    act_cfg=spatial_attn_act,
                    norm_cfg=spatial_attn_norm
                )
            )
    ...
    def forward(self, inputs):
        ...
        for idx in range(len(inputs) - 1, -1, -1):
            linear_prj = self.linear_projections[idx]
            spatial_attn = self.spatial_attns[idx]
            # cat first 2 from inputs
            if idx == len(inputs) - 1:
                x1 = inputs[idx]
                x2 = inputs[idx - 1]
...
# ExFuse + Add + Full-scale
# skip connection uses (1/32 + 1/4) instead of (1/8 + 1/4)
@HEADS.register_module()
class LAPHead_v2_40(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 **kwargs):
        super().__init__(
    ...
    def forward(self, inputs):
        ...
        out = self.se_module(out)
        out = self.fusion_conv(out)
        # perform identity mapping with (1/4 + 1/32)
        out = outs[-1] + out
...
# ExFuse + Add + Full-scale
# skip connection uses both (1/32 + 1/4) and (1/8 + 1/4)
@HEADS.register_module()
class LAPHead_v2_41(BaseDecodeHead):
    def __init__(self,
                 interpolate_mode='bilinear',
                 **kwargs):
        super().__init__(
...
# ExFuse + Add + Full-scale + Both-skip + Scale
@HEADS.register_module()
class LAPHead_v2_42(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        out = self.fusion_conv(out)
        # perform identity mapping with (1/4 + 1/32)
        out = self.skip_scale[0](outs[-1]) + self.skip_scale[1](outs[-2]) + out
